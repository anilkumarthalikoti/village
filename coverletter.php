...
<?php 
 ...
 $query="select f.id,f.firstname_k,s.id fileid ,s.regdate,(select group_concat(landsono ) from landdetails where id in(select landdetailsid from schemefilling_land where fillingid=".$fileid." )) survayno, concat((select cropname from cropitems where id= s.item1),''||(select cropname from cropitems where id= s.item2),''||(select cropname from cropitems where id= s.item3)) crops  from schemefilling s , farmerdetails f where s.regid= f.id and s.id=".$fileid.""; 
       
$result =$conn->query($query);
foreach($result as $letter){
?>
<div id="<?php echo $fileid; ?>">
Pleas make a inspection of Mr/Mrs <u><?php echo $letter["firstname_k"]; ?></u> with the farmer registration id<u> <?php echo $letter["id"]; ?></u>holding the scheme filling id <u><?php echo $letter["fileid"] ?></u> registerd on <?php $letter["regdate"] ?> with the land survayno <u> <?php echo $letter["survayno"] ?></u> for the crop's
