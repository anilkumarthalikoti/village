...
class medoo
{
    ...
    public function __construct($options = null)
    {
        try {
            ...
            switch ($type) {
                ...
                case 'mysql':
                    if ($this->socket) {
                        $dsn = $type . ':unix_socket=' . $this->socket . ';dbname=' . $this->
                            database_name;
                    } else {
                        $dsn = $type . ':host=' . $this->server . ($is_port ? ';port=' . $port : '') .
                ...
                case 'oracle':
                    $dbname = $this->server ? '//' . $this->server . ($is_port ? ':' . $port :
                        ':1521') . '/' . $this->database_name : $this->database_name;
                ...
                case 'mssql':
                    $dsn = strstr(PHP_OS, 'WIN') ? 'sqlsrv:server=' . $this->server . ($is_port ?
                        ',' . $port : '') . ';database=' . $this->database_name : 'dblib:host=' . $this->
                        server . ($is_port ? ':' . $port : '') . ';dbname=' . $this->database_name;
            ...
            if (in_array($type, explode(' ', 'mariadb mysql pgsql sybase mssql')) && $this->
                charset) {
    ...
    protected function column_push($columns)
    {
        ...
        foreach ($columns as $key => $value) {
            ...
            if (isset($match[1], $match[2])) {
                array_push($stack, $this->column_quote($match[1]) . ' AS ' . $this->
                    column_quote($match[2]));
            } else {
    ...
    protected function data_implode($data, $conjunctor, $outer_conjunctor = null)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            if (preg_match("/^(AND|OR)(\s+#.*)?$/i", $key, $relation_match) && $type ==
                'array') {
                $wheres[] = 0 !== count(array_diff_key($value, array_keys(array_keys($value)))) ?
                    '(' . $this->data_implode($value, ' ' . $relation_match[1]) . ')' : '(' . $this->
                    inner_conjunct($value, ' ' . $relation_match[1], $conjunctor) . ')';
            } else {
                preg_match('/(#?)([\w\.\-]+)(\[(\>|\>\=|\<|\<\=|\!|\<\>|\>\<|\!?~)\])?/i', $key,
                    $match);
                ...
                if (isset($match[4])) {
                    ...
                    if ($operator == '<>' || $operator == '><') {
                        if ($type == 'array') {
                            ...
                            if (is_numeric($value[0]) && is_numeric($value[1])) {
                                ...
                                $wheres[] = '(' . $column . ' BETWEEN ' . $this->quote($value[0]) . ' AND ' . $this->
                                    quote($value[1]) . ')';
                            }
                        }
                    }
                    ...
                    if ($operator == '~' || $operator == '!~') {
                        ...
                        foreach ($value as $item) {
                            ...
                            $like_clauses[] = $column . ($operator === '!~' ? ' NOT' : '') . ' LIKE ' . $this->
                                fn_quote($key, $item);
                        }
            ...
            if (isset($where['MATCH'])) {
                ...
                if (is_array($MATCH) && isset($MATCH['columns'], $MATCH['keyword'])) {
                    $where_clause .= ($where_clause != '' ? ' AND ' : ' WHERE ') . ' MATCH ("' .
                        str_replace('.', '"."', implode($MATCH['columns'], '", "')) . '") AGAINST (' . $this->
                        quote($MATCH['keyword']) . ')';
                }
            }
            ...
            if (isset($where['ORDER'])) {
                ...
                if (is_array($ORDER)) {
                    if (isset($ORDER[1]) && is_array($ORDER[1])) {
                        $where_clause .= ' ORDER BY FIELD(' . $this->column_quote($ORDER[0]) . ', ' . $this->
                            array_quote($ORDER[1]) . ')';
                    } else {
                        $stack = array();
                        ...
                        foreach ($ORDER as $column) {
                            ...
                            array_push($stack, '"' . str_replace('.', '"."', $order_match[1]) . '"' . (isset
                                ($order_match[3]) ? ' ' . $order_match[3] : ''));
                        }
                    ...
                    $where_clause .= ' ORDER BY "' . str_replace('.', '"."', $order_match[1]) . '"' . (isset
                        ($order_match[3]) ? ' ' . $order_match[3] : '');
                }
            }
    ...
    protected function select_context($table, $join, &$columns = null, $where = null,
        $column_fn = null)
    {
        ...
        if (isset($join_key[0]) && strpos($join_key[0], '[') === 0) {
            ...
            foreach ($join as $sub_table => $relation) {
                ...
                if ($match[2] != '' && $match[3] != '') {
                        ...
                        if (isset($relation[0])) {
                            ...
                            foreach ($relation as $key => $value) {
                                $joins[] = $this->prefix . (strpos($key, '.') > 0 ?
                                    ...
                                    $table . '."' . $key . '"') . ' = ' . '"' . (isset($match[5]) ? $match[5] : $match[3]) .
                                    '"."' . $value . '"';
                            }
                    ...
                    $table_join[] = $join_array[$match[2]] . ' JOIN "' . $this->prefix . $match[3] .
    ...
    public function select($table, $join, $columns = null, $where = null)
    {
        ...
        return $query ? $query->fetchAll((is_string($columns) && $columns != '*') ? PDO::
            FETCH_COLUMN : PDO::FETCH_ASSOC) : false;
    }
    ...
    public function insert($table, $datas)
    {
        ...
        foreach ($datas as $data) {
            ...
            foreach ($data as $key => $value) {
                ...
                switch (gettype($value)) {
                    ...
                    case 'array':
                        ...
                        $values[] = isset($column_match[0]) ? $this->quote(json_encode($value)) : $this->
                            quote(serialize($value));
                        break;
            ...
            $this->exec('INSERT INTO "' . $this->prefix . $table . '" (' . implode(', ', $columns) .
    ...
    public function update($table, $data, $where = null)
    {
            ...
            if (isset($match[3])) {
                ...
                switch (gettype($value)) {
                    ...
                    case 'array':
                        ...
                        $fields[] = $column . ' = ' . $this->quote(isset($column_match[0]) ? json_encode
        ...
        return $this->exec('UPDATE "' . $this->prefix . $table . '" SET ' . implode(', ',
            $fields) . $this->where_clause($where));
    }

    public function delete($table, $where)
    {
        return $this->exec('DELETE FROM "' . $this->prefix . $table . '"' . $this->
            where_clause($where));
    }
    ...
    public function replace($table, $columns, $search = null, $replace = null, $where = null)
    {
        ...
        return $this->exec('UPDATE "' . $this->prefix . $table . '" SET ' . $replace_query .
    ...
    public function has($table, $join, $where = null)
    {
        ...
        $query = $this->query('SELECT EXISTS(' . $this->select_context($table, $join, $column,
            $where, 1) . ')');
